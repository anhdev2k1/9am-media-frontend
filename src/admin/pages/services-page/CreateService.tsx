...
const CreateService = () => {
  ...
  const { mutate: Create, status, error } = useMutation({ mutationFn: (data) => handleQuery(data) })

  if (error) {
    return <Toast message={error.message} position='top-right' />
  }
  const handleCreate = async (values: any) => {
    const newData = queryKey.includes('services')
      ? { ...values, content: value, category: location?.[3] }
  ...
  return (
    <div className='flex flex-col gap-3 bg-white p-3 justify-center items-center min-h-[100vh]'>
      ...
      <h3 className='text-2xl text-black font-semibold uppercase pt-2 pb-3'>Tạo bài viết mới</h3>
      <Form
        form={formCreate}
        layout='vertical'
